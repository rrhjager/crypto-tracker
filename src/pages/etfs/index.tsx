...
export default function ETFsIndex() {
  ...
  // ✅ favorites (EQUITY-kind, market=ETFS)
  ...
  const favSet = useMemo(() => {
    ...
    return new Set(
      arr
        .filter((it: any) => String(it?.market || '').toUpperCase() === 'ETFS')
      ...
      ? current.filter(
          (it: any) =>
            !(
              ...
              String(it?.market || '').toUpperCase() === 'ETFS' &&
              String(it?.kind || '') === 'EQUITY'
            )
        )
      : [
          {
            id: `tmp:EQUITY:ETFS:${s}`,
            kind: 'EQUITY',
            symbol: s,
            market: 'ETFS',
    ...
    try {
      if (!isFav) {
        const r = await fetch('/api/user/favorites', {
          ...
          body: JSON.stringify({ kind: 'EQUITY', symbol: s, market: 'ETFS' }),
        })
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
      } else {
        const r = await fetch(
          `/api/user/favorites?kind=EQUITY&symbol=${encodeURIComponent(s)}&market=${encodeURIComponent('ETFS')}`,
          { method: 'DELETE' }
        )
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
      }
    ...
    async function load() {
      try {
        ...
        const parts = await pool(groups, 4, async (group, gi) => {
          ...
          const url = `/api/indicators/snapshot?symbols=${encodeURIComponent(group.join(','))}&market=ETFS`
  ...
  const summary = useMemo(() => {
    ...
    const priced = ETFS.map(a => ({ symbol: a.symbol, pct: Number(bySym[a.symbol]?.changePct) })).filter(x =>
    ...
    <>
      ...
      <main className="min-h-screen">
        ...
        <section className="max-w-6xl mx-auto px-4 pb-16">
          ...
          {/* ✅ same layout as favorites-enabled pages */}
