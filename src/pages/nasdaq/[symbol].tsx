...
export default function StockDetail() {
  ...
  const { data, error, isLoading } = useSWR<SnapResp>(
    symbol ? `/api/indicators/snapshot-list?symbols=${encodeURIComponent(symbol)}&market=NASDAQ` : null,
    fetcher,
  ...
  const { data: serverScoreData } = useSWR<ScoreResp>(
    symbol ? `/api/indicators/score/${encodeURIComponent(symbol)}?market=NASDAQ` : null,
    fetcher,
      ...
      <main className="min-h-screen">
        <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <StockIndicatorCard
              title="Trend (20d)"
              status={loading ? 'HOLD' : errMsg ? 'HOLD' : (trend20?.status || 'HOLD')}
              note={
                loading
                  ? 'Bezig met ophalen...'
                  : errMsg
            ...
            <StockIndicatorCard
              ...
              status={loading ? 'HOLD' : errMsg ? 'HOLD' : (volatility20?.status || 'HOLD')}
              note={
                loading
                  ? 'Bezig met ophalen...'
                  : errMsg
          ...
          <div className="mt-6 flex gap-3">
            <Link href="/nasdaq" className="btn">
              ← Back to NASDAQ list
            </Link>
