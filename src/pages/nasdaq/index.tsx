...
// batching helpers (zelfde beleid als eerder)
const CHUNK = 50
...
export default function Nasdaq() {
  ...
  // 2) 7d/30d returns (zelfde als voorheen, blijft zuinig door batching)
  ...
  const summary = useMemo(() => {
    const rows = NASDAQ.map(a => bySym[a.symbol]).filter(Boolean) as SnapItem[]
    ...
    const priced = NASDAQ.map(a => ({ symbol:a.symbol, pct: Number(bySym[a.symbol]?.changePct) }))
                  ...
                  {NASDAQ.map((row, i) => {
                    ...
                    return (
                      <tr key={row.symbol} className="hover:bg-gray-50 dark:hover:bg-slate-800/60">
                        <td className="px-3 md:px-4 py-3 text-gray-500 dark:text-slate-400">{i+1}</td>
                        <td className="px-3 md:px-4 py-3">
                          <div className="flex items-center gap-2">
                            <Link
                              href={`/nasdaq/${encodeURIComponent(row.symbol)}`}
                        ...
                        <td className="px-3 md:px-4 py-3 text-gray-900 dark:text-slate-100">{price}</td>
