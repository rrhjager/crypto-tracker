// src/pages/api/past-performance/sp500.ts
...
// small Yahoo symbol cleanup for rare class tickers like BRK.B -> BRK-B
...
async function computeRow(symbol: string, name: string): Promise<Row> {
  try {
    ...
    // avoid early “junk” (no MA200 yet), start searching only after 220 bars
    const MIN_I = 220
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const symbols = SP500.map(x => ({ symbol: x.symbol, name: x.name }))
    const rows: Row[] = []
    ...
    res.status(200).json({
      meta: { market: 'SP500', computedAt: Date.now() },
      rows,
    })
  } catch (e: any) {
