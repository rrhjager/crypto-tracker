...
/* ---------- KV → SnapItem (statusvelden from numbers) ---------- */
type KVShape = {
  symbol: string
  ma?: { ma50: number|null; ma200: number|null; status?: Advice }
  rsi?: number|null
...
function toSnapFromKV(sym: string, v?: KVShape|null): SnapItem | null {
  ...
  const maS  = (ma50!=null && ma200!=null) ? (ma50>ma200?'BUY':ma50<ma200?'SELL':'HOLD') : (v.ma?.status ?? 'HOLD')

  const rsiVal = typeof v.rsi === 'number' ? v.rsi : null
  const rsiS   = adv(rsiVal, 30, 70)

  const m = v.macd ?? { macd:null, signal:null, hist:null }
  const macdS = (m.macd!=null && m.signal!=null) ? (m.macd>m.signal?'BUY':m.macd<m.signal?'SELL':'HOLD') : 'HOLD'
  ...
  const score = Number.isFinite(v.score as number) ? Math.round(Number(v.score)) : scoreFrom(maS, macdS, rsiS, volS)
  ...
  return {
    ...
    volume: { volume: vol.volume ?? null, avg20d: vol.avg20d ?? null, ratio, status: volS },
    score
  }
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Resp|{error:string}>) {
  ...
  try {
    ...
    for (const sym of symbols) {
      ...
      if (mapped) {
        items.push(mapped)
        if (cached?.updatedAt && cached.updatedAt > newestUpdatedAt) newestUpdatedAt = cached.updatedAt
      } else {
        misses.push(sym)
      }
    }
    ...
    return res.status(200).json({ items, updatedAt: newestUpdatedAt || Date.now() })
  } catch (e:any) {
