...
// ——— gedeelde compute-functie (zodat kvRefreshIfStale dezelfde logica gebruikt) ———
async function computeScore(symbol: string): Promise<Resp> {
  const ohlc = await getYahooDailyOHLC(symbol, '1y') // { timestamps, closes, volumes } of array van candles
  ...
  const vol = vol20(volumes)            // { period, volume?, avg20?, ratio, status?, points? }

  // normaliseer
  const ma50 = (ma as any)?.ma50 ?? null
  const ma200 = (ma as any)?.ma200 ?? null
  const rsiValue =
    typeof (rsi as any)?.rsi === 'number' ? (rsi as any).rsi : (typeof rsi === 'number' ? rsi : null)
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (cached?.value && cached.updatedAt && (now - cached.updatedAt < TTL_SEC * 1000)) {
      // serve warm; plan lazy refresh als verouderd
      kvRefreshIfStale(
        key,
        REVALIDATE_SEC,
        async () => {
          const value = await computeScore(symbol)
          return { updatedAt: Date.now(), value } as Snap
        },
        TTL_SEC
      ).catch(() => {})
    ...
    // geen (geldige) cache -> berekenen en zetten
    const value = await computeScore(symbol)
    await kvSetJSON(key, { updatedAt: now, value }, TTL_SEC)
