...
// “Up to date” definitie (voor headers/diagnose)
const FRESH_MS = 5 * 60_000

// Belangrijk: KV mag niet “leegvallen” → lange TTL (failsafe).
// Cron ververst elke 5 min, maar TTL is bv 24h zodat bezoekers altijd direct data krijgen.
...
async function buildSnapshot(origin: string): Promise<Snapshot> {
  ...
  // ---- News + Academy + Congress parallel ----
  ...
  // ---- Equities top BUY/SELL per markt (parallel per markt) ----
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // --------- Warmup modus (blijft, breekt niets) ----------
    ...
    // 0) Alleen cron/internal mag rebuilden (voorkomt “visitor cold build”)
    if (refresh) {
      if (!isAuthorized(req)) return res.status(401).json({ error: 'Unauthorized' })
    ...
    if (CACHE && Date.now() - CACHE.ts < MEM_TTL_MS) {
      const age = Date.now() - (CACHE.payload.updatedAt || 0)
      res.setHeader('Cache-Control', 'public, s-maxage=60, stale-while-revalidate=300')
    ...
    // 3) Eerste keer ooit (KV leeg) → éénmalig build + store
    // (daarna neemt cron het over)
    const value = await buildSnapshot(origin)
    ...
    res.setHeader('Cache-Control', 'public, s-maxage=60, stale-while-revalidate=300')
    res.setHeader('x-home-snapshot-age-ms', '0')
    res.setHeader('x-home-snapshot-stale', '0')
    return res.status(200).json(value)
  } catch (err: any) {
