...
// DeFiLlama — voor yield (blijft staan)
...
// ── Baseline APY’s voor FAST/fallback
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // ★ Belangrijk: default FAST = true als er geen param is (snelle pad bij cold start).
    ...
    const prelim: Pre[] = await Promise.all(
      klinesByCoin.map(async ({ coin, closes1h, closes1d, _livePrice, _priceSrc }) => {
        ...
        // YIELD
        // - In FAST: geen externe calls; baseline per symbool (indien bekend).
        // - In deep (fast=0): haal pools via DeFiLlama; filter op kwaliteit; fallback naar baseline.
        let pools: any[] = [];
        let bestApyEff: number | null = null;

        if (!FAST) {
          const list = await safe(topPoolsForSymbol(coin.symbol, { minTvlUsd: 3_000_000, maxPools: 6 }) as any, []);
          ...
          for (const p of pools) {
            ...
            let qual = 1;
            if (p?.stablecoin === true) qual *= 0.85; // iets lager gewicht
            if (hasIlRisk(p))         qual *= 0.70; // IL risico
            const eff = apy * qual;
        ...
        if (bestApyEff == null) {
          const base = BASELINE_APY[coin.symbol.toUpperCase()];
          if (typeof base === "number") bestApyEff = base;
        }
    ...
    // 6) Yield percentielen (gebruik zowel deep als baseline → altijd gevuld)
    ...
    const results = prelim.map((p, i) => {
      ...
      // Yield-score (altijd mogelijk via baseline/deep)
