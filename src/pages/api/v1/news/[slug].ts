...
// Consistente CDN-vriendelijke headers
...
export default async function handler(
  ...
  const symbol = String(Array.isArray(rawSlug) ? rawSlug[0] : rawSlug || "").toUpperCase();
  const base = nameParam && nameParam.length > 0 ? nameParam : symbol || "crypto";
  ...
  try {
    ...
    // Robuuste fetch (timeout + 1 retry), geen cache
    const r = await fetchSafe(url, { cache: "no-store" }, 7000, 1);
    if (!r.ok) {
      setCacheHeaders(res, 300, 120);
      return res.status(200).json({ updatedAt: Date.now(), query, items: [] });
    }

    const xml = await r.text();
    const feed = await parser.parseString(xml);

    const items: NewsItem[] = (feed.items || []).slice(0, 12).map((it, idx) => {
      // bron opvangen uit mogelijke velden, fallback op host uit link
      ...
      const pubDate =
        (it as any).isoDate ||
        (it.pubDate ? new Date(it.pubDate).toISOString() : undefined);
    ...
    // Graceful fallback: 200 met lege lijst i.p.v. 500
