...
export default function Dax() {
  ...
  const favSet = useMemo(() => {
    ...
    return new Set(
      arr
        .filter((it: any) => String(it?.market || '').toUpperCase() === 'DAX')
      ...
      ? current.filter(
          (it: any) =>
            !(
              ...
              String(it?.market || '').toUpperCase() === 'DAX' &&
    ...
    try {
      if (!isFav) {
        const r = await fetch('/api/user/favorites', {
          ...
          body: JSON.stringify({ kind: 'EQUITY', symbol: s, market: 'DAX' }),
        })
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
      } else {
        const r = await fetch(
          `/api/user/favorites?kind=EQUITY&symbol=${encodeURIComponent(s)}&market=${encodeURIComponent('DAX')}`,
          { method: 'DELETE' }
        )
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
      }
    ...
    async function load() {
      try {
        ...
        const parts = await pool(groups, 4, async (group, gi) => {
          ...
          const url = `/api/indicators/snapshot?symbols=${encodeURIComponent(group.join(','))}&market=DAX`
  ...
  const summary = useMemo(() => {
    ...
    const priced = DAX.map(a => ({ symbol: a.symbol, pct: Number(bySym[a.symbol]?.changePct) })).filter(x =>
  ...
  return (
    <>
      <Head>
        <title>DAX Tracker — SignalHub</title>
      </Head>

      <main className="min-h-screen">
        <section className="max-w-6xl mx-auto px-4 pt-16 pb-8">
          <h1 className="hero">DAX Tracker</h1>
        </section>

        <section className="max-w-6xl mx-auto px-4 pb-16">
          {snapErr && <div className="mb-3 text-red-600 text-sm">Fout bij laden: {snapErr}</div>}

          {/* ✅ same layout as AEX/NASDAQ/SP500/Dow */}
                      ...
                      <tr key={row.symbol} className="hover:bg-gray-50 dark:hover:bg-slate-800/60">
                        ...
                        <td className="px-2 py-3">
                          <div className="flex items-center gap-2">
                            <Link
                              href={`/dax/${encodeURIComponent(row.symbol)}`}
                ...
                <div className="mt-3 grid grid-cols-4 sm:grid-cols-5 gap-2">
                  {heatmapData.map(({ symbol, score, status }) => {
                    ...
                    return (
                      <Link
                        key={symbol}
                        href={`/dax/${encodeURIComponent(symbol)}`}
