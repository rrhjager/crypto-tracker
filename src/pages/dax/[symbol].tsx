...
export default function StockDetail() {
  ...
  const { data, error, isLoading } = useSWR<SnapResp>(
    symbol ? `/api/indicators/snapshot-list?symbols=${encodeURIComponent(symbol)}&market=DAX` : null,
    fetcher,
  ...
  const { data: serverScoreData } = useSWR<ScoreResp>(
    symbol ? `/api/indicators/score/${encodeURIComponent(symbol)}?market=DAX` : null,
    fetcher,
