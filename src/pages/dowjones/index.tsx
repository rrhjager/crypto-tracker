...
export default function DowJones() {
  ...
  const favSet = useMemo(() => {
    ...
    return new Set(
      arr
        .filter((it: any) => String(it?.market || '').toUpperCase() === 'DOWJONES')
      ...
      ? current.filter(
          (it: any) =>
            !(
              ...
              String(it?.market || '').toUpperCase() === 'DOWJONES' &&
              String(it?.kind || '') === 'EQUITY'
            )
        )
      : [
          {
            id: `tmp:EQUITY:DOWJONES:${s}`,
            kind: 'EQUITY',
            symbol: s,
            market: 'DOWJONES',
    ...
    try {
      if (!isFav) {
        const r = await fetch('/api/user/favorites', {
          ...
          body: JSON.stringify({ kind: 'EQUITY', symbol: s, market: 'DOWJONES' }),
        })
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
      } else {
        const r = await fetch(
          `/api/user/favorites?kind=EQUITY&symbol=${encodeURIComponent(s)}&market=${encodeURIComponent('DOWJONES')}`,
          { method: 'DELETE' }
        )
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
      }
    ...
    async function load() {
      try {
        ...
        const parts = await pool(groups, 4, async (group, gi) => {
          ...
          const url = `/api/indicators/snapshot?symbols=${encodeURIComponent(group.join(','))}&market=DOWJONES`
  ...
  const summary = useMemo(() => {
    ...
    const priced = DOWJONES.map(a => ({ symbol: a.symbol, pct: Number(bySym[a.symbol]?.changePct) })).filter(x =>
  ...
  return (
    <>
      <Head>
        <title>Dow Jones Tracker — SignalHub</title>
      </Head>
      ...
      <main className="min-h-screen">
        ...
        <section className="max-w-6xl mx-auto px-4 pb-16">
          ...
          {/* ✅ same layout as AEX/NASDAQ/SP500 */}
