...
// batching helpers (consistent met NASDAQ/SP500)
const CHUNK = 50
...
export default function DowJones() {
  const symbols = useMemo(() => DOWJONES.map(x => x.symbol), [])

  // 1) Snapshot (batches, alles uit één endpoint)
  ...
  const summary = useMemo(() => {
    const rows = DOWJONES.map(a => bySym[a.symbol]).filter(Boolean) as SnapItem[]
    ...
    const priced = DOWJONES.map(a => ({ symbol:a.symbol, pct: Number(bySym[a.symbol]?.changePct) }))
  ...
  return (
    <>
      <Head><title>Dow Jones Tracker — SignalHub</title></Head>

      <main className="min-h-screen">
        <section className="max-w-6xl mx-auto px-4 pt-16 pb-8">
          <h1 className="hero">Dow Jones Tracker</h1>
        </section>
                      ...
                      <tr key={row.symbol} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <Link
                              href={`/dowjones/${encodeURIComponent(row.symbol)}`}
              ...
              <div className="table-card p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    <button className={`px-2.5 py-1 rounded-full text-xs border ${filter==='ALL'
                      ? 'bg-gray-200 text-gray-900 border-gray-300'
                      : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`} onClick={() => setFilter('ALL')}>All</button>
                    <button className={`px-2.5 py-1 rounded-full text-xs border ${filter==='BUY'
                      ? 'bg-green-600 text-white border-green-600'
                      : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`} onClick={() => setFilter('BUY')}>Buy</button>
                    <button className={`px-2.5 py-1 rounded-full text-xs border ${filter==='HOLD'
                      ? 'bg-amber-500 text-white border-amber-500'
                      : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`} onClick={() => setFilter('HOLD')}>Hold</button>
                    <button className={`px-2.5 py-1 rounded-full text-xs border ${filter==='SELL'
                      ? 'bg-red-600 text-white border-red-600'
                      : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`} onClick={() => setFilter('SELL')}>Sell</button>
                  </div>
                </div>
                ...
                <div className="mt-3 grid grid-cols-4 sm:grid-cols-5 gap-2">
                  {heatmapData.map(({ symbol, score, status }) => {
                    ...
                    return (
                      <Link
                        key={symbol}
                        href={`/dowjones/${encodeURIComponent(symbol)}`}
