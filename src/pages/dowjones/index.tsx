...
export default function DowJones() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const list = DOWJONES.map(x => x.symbol)
  ...
  const summary = useMemo(() => {
    const withScore = DOWJONES.map(a => ({ sym: a.symbol, s: scoreMap[a.symbol] })).filter(x => Number.isFinite(x.s))
    ...
    const pctArr = DOWJONES.map(a => Number(quotes[a.symbol]?.regularMarketChangePercent))
      ...
      <main className="min-h-screen">
        ...
        <section className="max-w-6xl mx-auto px-4 pb-16">
          ...
          <div className="grid lg:grid-cols-[2fr_1fr] gap-4">
            {/* Lijst in dezelfde crypto-stijl tabel */}
