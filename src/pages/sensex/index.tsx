...
export default function Sensex() {
  ...
  return (
    <>
      ...
      {/* Compacte typografie alléén hier */}
      <main className="min-h-screen sensex-page text-[13px]">
        <section className="max-w-6xl mx-auto px-4 pt-16 pb-6">
          {/* ⬇️ Alleen deze regel aangepast: grotere titel zoals elders */}
          <h1 className="hero">Sensex</h1>
        </section>

        <section className="max-w-6xl mx-auto px-4 pb-14">
          {qErr && <div className="mb-3 text-red-600 text-xs">Fout bij laden quotes: {qErr}</div>}
          ...
          <div className="grid lg:grid-cols-[2fr_1fr] gap-4">
              ...
              <table className="w-full text-[13px]">
                <thead className="bg-gray-50">
                  <tr className="text-left text-gray-500 text-[11px]">
                    ...
                    <th className="px-3 py-2.5">Aandeel</th>
                    <th className="px-3 py-2.5">Prijs</th>
                    <th className="px-3 py-2.5">24h</th>
                    <th className="px-3 py-2.5">7d</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {AEX.map((row, i) => {
                    ...
                    const price = fmtPrice(q?.regularMarketPrice, q?.currency || 'INR')
                      ...
                      <tr key={row.symbol} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2.5">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-gray-500 text-[12px]">({row.symbol})</span>
                          </div>
                        </td>
                        <td className="px-3 py-2.5 text-gray-900">{price}</td>
                        <td className={`px-3 py-2.5 ${pctCls(pct)}`}>
                          {Number.isFinite(chg as number) && Number.isFinite(pct as number)
                            ? `${chg! >= 0 ? '+' : ''}${num(chg, 2)} (${pct! >= 0 ? '+' : ''}${num(pct, 2)}%)`
                            : '—'}
                        </td>
                        <td className={`px-3 py-2.5 ${pctCls(r7)}`}>
                          {Number.isFinite(r7 as number) ? `${(r7 as number) >= 0 ? '+' : ''}${num(r7, 2)}%` : '—'}
                        </td>
                        <td className={`px-3 py-2.5 ${pctCls(r30)}`}>
                          {Number.isFinite(r30 as number) ? `${(r30 as number) >= 0 ? '+' : ''}${num(r30, 2)}%` : '—'}
                        </td>
                        <td className="px-3 py-2.5">
                          <div className="flex justify-end">
                            {Number.isFinite(score as number)
                              ? <span className="scale-95 origin-right"><ScoreBadge score={score as number} /></span>
                              : <span className="badge badge-hold text-[10px]">HOLD · 50</span>}
                          </div>
                        </td>
                      </tr>
                    )
            ...
            <aside className="space-y-3 lg:sticky lg:top-16 h-max text-[13px]">
              {/* Dagelijkse samenvatting */}
              <div className="table-card p-3">
                <div className="flex items-center justify-between mb-2">
                  <div className="font-semibold text-gray-900 text-[13px]">Dagelijkse samenvatting</div>
                ...
                <div className="grid grid-cols-3 gap-2 mb-2">
                  <div className="rounded-xl border border-green-500/30 bg-green-500/10 p-2.5 text-center">
                  ...
                  <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-2.5 text-center">
                    <div className="text-[11px] text-gray-600">HOLD</div>
                ...
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <div className="rounded-xl border border-gray-200 p-2.5">
                    <div className="text-[11px] text-gray-600">Breadth (24h groen)</div>
                    <div className="text-lg font-bold text-gray-900">{summary.breadthPct}%</div>
                  </div>
                  <div className="rounded-xl border border-gray-200 p-2.5">
                    <div className="text-[11px] text-gray-600">Gem. score</div>
                    <div className="text-lg font-bold text-gray-900">{summary.avgScore}</div>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-xl border border-gray-200 p-2.5">
                    <div className="text-[11px] text-gray-600 mb-1">Top stijgers (24h)</div>
                  ...
                  <div className="rounded-xl border border-gray-200 p-2.5">
                    <div className="text-[11px] text-gray-600 mb-1">Top dalers (24h)</div>
                    <ul className="space-y-1 text-[12px]">
              ...
              <div className="table-card p-3">
                <div className="flex items-center justify-between">
                  <div className="font-semibold text-gray-900 text-[13px]">Heatmap</div>
                  <div className="flex gap-1">
                    <button
                      className={`px-2 py-1 rounded-full text-[11px] border ${filter==='ALL'
                        ? 'bg-gray-200 text-gray-900 border-gray-300'
                        : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
                      onClick={() => setFilter('ALL')}
                    >All</button>
                    <button
                      className={`px-2 py-1 rounded-full text-[11px] border ${filter==='BUY'
                        ? 'bg-green-600 text-white border-green-600'
                        : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
                      onClick={() => setFilter('BUY')}
                    >Buy</button>
                    <button
                      className={`px-2 py-1 rounded-full text-[11px] border ${filter==='HOLD'
                        ? 'bg-amber-500 text-white border-amber-500'
                        : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
                      onClick={() => setFilter('HOLD')}
                    >Hold</button>
                    <button
                      className={`px-2 py-1 rounded-full text-[11px] border ${filter==='SELL'
                ...
                <div className="mt-3 grid grid-cols-4 sm:grid-cols-5 gap-2">
                  {heatmapData.map(({ symbol, score, status }) => {
                    // Toon eerste 3 letters van de AANDEELNAAM (fallback: symbool)
                    const name = AEX.find(a => a.symbol === symbol)?.name || symbol
                    const label = (name.replace(/[^A-Za-z]/g, '') || name).slice(0, 3).toUpperCase()
                    ...
                    return (
                      <Link
                        ...
                        className={`rounded-xl px-2 py-1.5 text-[11px] font-semibold border text-center hover:opacity-90 ${cls}`}
                        title={`${name} (${symbol}) · ${status} · ${score}`}
                      >
                        <div className="leading-none tracking-wide">{label}</div>
