...
export default function Sensex() {
  ...
  const favSet = useMemo(() => {
    ...
    return new Set(
      arr
        .filter((it: any) => String(it?.market || '').toUpperCase() === 'SENSEX')
      ...
      ? current.filter(
          (it: any) =>
            !(
              ...
              String(it?.market || '').toUpperCase() === 'SENSEX' &&
              String(it?.kind || '') === 'EQUITY'
            )
        )
      : [
          {
            id: `tmp:EQUITY:SENSEX:${s}`,
            kind: 'EQUITY',
            symbol: s,
            market: 'SENSEX',
    ...
    try {
      if (!isFav) {
        const r = await fetch('/api/user/favorites', {
          ...
          body: JSON.stringify({ kind: 'EQUITY', symbol: s, market: 'SENSEX' }),
        })
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
      } else {
        const r = await fetch(
          `/api/user/favorites?kind=EQUITY&symbol=${encodeURIComponent(s)}&market=${encodeURIComponent('SENSEX')}`,
          { method: 'DELETE' }
        )
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
      }
    ...
    async function load() {
      try {
        ...
        const parts = await pool(groups, 4, async (group, gi) => {
          ...
          const url = `/api/indicators/snapshot?symbols=${encodeURIComponent(group.join(','))}&market=SENSEX`
                      ...
                      <tr key={row.symbol} className="hover:bg-gray-50 dark:hover:bg-slate-800/60 align-middle">
                        ...
                        <td className="px-2 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-gray-500 dark:text-slate-400 shrink-0">({disp(row.symbol)})</span>
                          </div>
                        </td>
                    ...
                    <ul className="space-y-1 text-sm">
                      {summary.topGainers.map((g, i) => (
                        <li key={`g${i}`} className="flex justify-between">
                          <span className="text-gray-800 dark:text-slate-100">{disp(g.symbol)}</span>
                    ...
                    <ul className="space-y-1 text-sm">
                      {summary.topLosers.map((l, i) => (
                        <li key={`l${i}`} className="flex justify-between">
                          <span className="text-gray-800 dark:text-slate-100">{disp(l.symbol)}</span>
