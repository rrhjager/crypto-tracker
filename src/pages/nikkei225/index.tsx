...
function fmtPrice(v: number | null | undefined, ccy?: string) {
  ...
  try {
    if (ccy) return new Intl.NumberFormat('ja-JP', { style: 'currency', currency: ccy }).format(v as number)
  } catch {}
...
export default function Nikkei225() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const list = NIKKEI225.map(x => x.symbol)
  ...
  const summary = useMemo(() => {
    const withScore = NIKKEI225.map(a => ({ sym: a.symbol, s: scoreMap[a.symbol] })).filter(x => Number.isFinite(x.s))
    ...
    const pctArr = NIKKEI225.map(a => Number(quotes[a.symbol]?.regularMarketChangePercent))
  ...
  return (
    <>
      <Head><title>Nikkei 225 Tracker — SignalHub</title></Head>
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr className="text-left text-gray-500">
                    <th className="px-3 md:px-4 py-3 w-8 md:w-10">#</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {NIKKEI225.map((row, i) => {
                    ...
                    const price = fmtPrice(q?.regularMarketPrice, q?.currency || 'JPY')
                      ...
                      <tr key={row.symbol} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 md:px-4 py-3">
                          <div className="flex items-center gap-2">
                            <Link href={`/nikkei225/${encodeURIComponent(row.symbol)}`} className="font-medium text-gray-900 hover:underline">
                              {row.name}
                            </Link>
                    ...
                    <ul className="space-y-1 text-sm">
                      {summary.topGainers.map((g, i) => (
                        <li key={`g${i}`} className="flex justify-between">
                          <span className="text-gray-800">{g.symbol.replace('.T','')}</span>
                    ...
                    <ul className="space-y-1 text-sm">
                      {summary.topLosers.map((l, i) => (
                        <li key={`l${i}`} className="flex justify-between">
                          <span className="text-gray-800">{l.symbol.replace('.T','')}</span>
