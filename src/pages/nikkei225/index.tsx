...
function fmtPrice(v: number | null | undefined, ccy: string = 'JPY') {
  ...
  try { return new Intl.NumberFormat('ja-JP', { style: 'currency', currency: ccy }).format(v as number) } catch {}
...
export default function Nikkei225() {
  ...
  const summary = useMemo(() => {
    ...
    const priced = NIKKEI225.map(a => ({ symbol:a.symbol, pct: Number(bySym[a.symbol]?.changePct) }))
