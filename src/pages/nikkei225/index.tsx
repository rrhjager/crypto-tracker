...
function fmtPrice(v: number | null | undefined, ccy: string = 'JPY') {
  ...
  return (v as number).toLocaleString('ja-JP', {
...
export default function Nikkei225() {
    ...
    async function load() {
      try {
        ...
        const parts = await pool(groups, 4, async (group, gi) => {
          ...
          const url = `/api/indicators/snapshot?symbols=${encodeURIComponent(
            group.join(',')
          )}`
  ...
  const summary = useMemo(() => {
    ...
    const pctArr = rows
      .map(r => Number(r.changePct))
      .filter(v => Number.isFinite(v)) as number[]
    const green = pctArr.filter(v => v > 0).length
    const breadthPct = pctArr.length
      ? Math.round((green / pctArr.length) * 100)
      : 0

    const priced = NIKKEI225.map(a => ({
      symbol: a.symbol,
      pct: Number(bySym[a.symbol]?.changePct),
    })).filter(x => Number.isFinite(x.pct)) as {
      symbol: string
      pct: number
    }[]
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-slate-800/80">
                  {NIKKEI225.map((row, i) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-3 md:px-4 py-3 text-gray-500 dark:text-slate-400">
                          {i + 1}
                        </td>
                        <td className="px-3 md:px-4 py-3">
                          <div className="flex items-center gap-2">
                            <Link
                              href={`/nikkei225/${encodeURIComponent(
                                row.symbol
                              )}`}
                        ...
                        <td className="px-3 md:px-4 py-3 text-gray-900 dark:text-slate-100">
                          {price}
                        </td>
