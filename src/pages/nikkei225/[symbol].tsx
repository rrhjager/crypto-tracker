...
export default function StockDetail() {
  ...
  const { data, error, isLoading } = useSWR<SnapResp>(
    symbol ? `/api/indicators/snapshot-list?symbols=${encodeURIComponent(symbol)}&market=NIKKEI225` : null,
    fetcher,
  ...
  const { data: serverScoreData } = useSWR<ScoreResp>(
    symbol ? `/api/indicators/score/${encodeURIComponent(symbol)}?market=NIKKEI225` : null,
    fetcher,
  ...
  const combinedScore = serverScore ?? fallbackScore ?? 50
