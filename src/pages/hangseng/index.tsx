// src/pages/hangseng/index.tsx
import Head from 'next/head'
import Link from 'next/link'
import { useEffect, useMemo, useState } from 'react'
import { useSession } from 'next-auth/react'
import useSWR from 'swr'
// We houden je alias aan voor consistente lijstimport:
...
function fmtPrice(v: number | null | undefined, ccy: string = 'HKD') {
...
export default function HangSeng() {
  ...
  const favSet = useMemo(() => {
    ...
    return new Set(
      arr
        .filter((it: any) => String(it?.market || '').toUpperCase() === 'HANGSENG')
      ...
      ? current.filter(
          (it: any) =>
            !(
              ...
              String(it?.market || '').toUpperCase() === 'HANGSENG' &&
              String(it?.kind || '') === 'EQUITY'
            )
        )
      : [
          {
            id: `tmp:EQUITY:HANGSENG:${s}`,
            kind: 'EQUITY',
            symbol: s,
            market: 'HANGSENG',
    ...
    try {
      if (!isFav) {
        const r = await fetch('/api/user/favorites', {
          ...
          body: JSON.stringify({ kind: 'EQUITY', symbol: s, market: 'HANGSENG' }),
        })
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
      } else {
        const r = await fetch(
          `/api/user/favorites?kind=EQUITY&symbol=${encodeURIComponent(s)}&market=${encodeURIComponent('HANGSENG')}`,
          { method: 'DELETE' }
        )
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
      }
  ...
  // 1) Snapshot (in batches) — vervangt quotes + losse indicator calls
    ...
    async function load() {
      try {
        ...
        const parts = await pool(groups, 4, async (group, gi) => {
          ...
          const url = `/api/indicators/snapshot?symbols=${encodeURIComponent(group.join(','))}&market=HANGSENG`
  ...
  // Samenvatting + top movers (uit snapshot)
  const summary = useMemo(() => {
    ...
    const priced = AEX.map(a => ({ symbol: a.symbol, pct: Number(bySym[a.symbol]?.changePct) })).filter(x =>
  ...
  return (
    <>
      <Head>
        <title>Hang Seng Index — SignalHub</title>
      </Head>

      <main className="min-h-screen">
        <section className="max-w-6xl mx-auto px-4 pt-16 pb-8">
          <h1 className="hero">Hang Seng Index</h1>
        </section>
              ...
              <table className="w-full text-[13px] table-fixed">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-slate-800/80">
                  {AEX.map((row, i) => {
                    ...
                    const price = fmtPrice(it?.price, 'HKD')
                    const chg = it?.change
                      ...
                      <tr key={row.symbol} className="hover:bg-gray-50 dark:hover:bg-slate-800/60 align-middle">
                        ...
                        <td className="px-2 py-3">
                          <div className="flex items-center gap-2">
                            <Link
                              href={`/hangseng/${encodeURIComponent(row.symbol)}`}
                    ...
                    <ul className="space-y-1 text-sm">
                      {summary.topGainers.map((g, i) => (
                        <li key={`g${i}`} className="flex justify-between">
                          <span className="text-gray-800 dark:text-slate-100">{g.symbol.replace('.HK', '')}</span>
                    ...
                    <ul className="space-y-1 text-sm">
                      {summary.topLosers.map((l, i) => (
                        <li key={`l${i}`} className="flex justify-between">
                          <span className="text-gray-800 dark:text-slate-100">{l.symbol.replace('.HK', '')}</span>
