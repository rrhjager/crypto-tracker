...
export default function StockDetail() {
  ...
  const meta = useMemo(() => HANGSENG.find(t => t.symbol === symbol), [symbol])

  // 1) snapshot-list (indicatoren + (na API-fix) score)
  const { data, error, isLoading } = useSWR<SnapResp>(
    symbol ? `/api/indicators/snapshot-list?symbols=${encodeURIComponent(symbol)}&market=HANGSENG` : null,
    fetcher,
  ...
  const { data: serverScoreData } = useSWR<ScoreResp>(
    symbol ? `/api/indicators/score/${encodeURIComponent(symbol)}?market=HANGSENG` : null,
    fetcher,
