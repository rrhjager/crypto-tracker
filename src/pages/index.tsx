...
export default function Homepage(props: HomeProps) {
  ...
  // Klein prefetch-tje voor navigatie-snelheid, heeft geen invloed op SSR-render
  useEffect(() => {
    const routes = [
      '/crypto',
  ...
  // Fallback als snapshot om wat voor reden dan ook ontbreekt
  if (!snapshot) {
    return (
      <>
        <Head>
        ...
        <main className="max-w-screen-2xl mx-auto px-4 pt-8 pb-14">
          <div className="text-white/70 text-sm">
            Homepage data is warming up… please refresh in a moment.
          </div>
        </main>
      </>
    )
  ...
  const briefingText = props.briefing?.advice || ''
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          ...
          <Card title="Daily AI Briefing">
            <div className={`flex-1 overflow-y-auto ${CARD_CONTENT_H} pr-1`}>
              {briefingText ? (
                <BriefingText text={briefingText} />
              ) : (
