...
function fmtPrice(v: number | null | undefined, ccy: string = 'GBP') {
...
// batching helpers (zelfde beleid overal)
const CHUNK = 50
...
export default function FTSEIndex() {
  const symbols = useMemo(() => FTSE100.map(x => x.symbol), [])
  ...
  const favSet = useMemo(() => {
    ...
    return new Set(
      arr
        .filter((it: any) => String(it?.market || '').toUpperCase() === 'FTSE100')
      ...
      ? current.filter(
          (it: any) =>
            !(
              ...
              String(it?.market || '').toUpperCase() === 'FTSE100' &&
              String(it?.kind || '') === 'EQUITY'
            )
        )
      : [
          {
            id: `tmp:EQUITY:FTSE100:${s}`,
            kind: 'EQUITY',
            symbol: s,
            market: 'FTSE100',
    ...
    try {
      if (!isFav) {
        const r = await fetch('/api/user/favorites', {
          ...
          body: JSON.stringify({ kind: 'EQUITY', symbol: s, market: 'FTSE100' }),
        })
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
      } else {
        const r = await fetch(
          `/api/user/favorites?kind=EQUITY&symbol=${encodeURIComponent(s)}&market=${encodeURIComponent('FTSE100')}`,
          { method: 'DELETE' }
        )
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
      }
    ...
    async function load() {
      try {
        ...
        const parts = await pool(groups, 4, async (group, gi) => {
          ...
          const url = `/api/indicators/snapshot?symbols=${encodeURIComponent(group.join(','))}&market=FTSE100`
  ...
  const summary = useMemo(() => {
    const rows = FTSE100.map(a => bySym[a.symbol]).filter(Boolean) as SnapItem[]
    ...
    const priced = FTSE100.map(a => ({ symbol: a.symbol, pct: Number(bySym[a.symbol]?.changePct) })).filter(x =>
  ...
  return (
    <>
      <Head>
        <title>FTSE 100 Tracker — SignalHub</title>
      </Head>

      <main className="min-h-screen">
        <section className="max-w-6xl mx-auto px-4 pt-16 pb-8">
          <h1 className="hero">FTSE 100 Tracker</h1>
        </section>
              ...
              <table className="w-full text-[13px] table-fixed">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-slate-800/80">
                  {FTSE100.map((row, i) => {
                    ...
                    const price = fmtPrice(it?.price, 'GBP')
                    const chg = it?.change
                      ...
                      <tr key={row.symbol} className="hover:bg-gray-50 dark:hover:bg-slate-800/60">
                        ...
                        <td className="px-2 py-3">
                          <div className="flex items-center gap-2">
                            <Link
                              href={`/ftse100/${encodeURIComponent(row.symbol)}`}
                    ...
                    <ul className="space-y-1 text-sm">
                      {summary.topGainers.map((g, i) => (
                        <li key={`g${i}`} className="flex justify-between">
                          <span className="text-gray-800 dark:text-slate-100">{g.symbol.replace('.L', '')}</span>
                    ...
                    <ul className="space-y-1 text-sm">
                      {summary.topLosers.map((l, i) => (
                        <li key={`l${i}`} className="flex justify-between">
                          <span className="text-gray-800 dark:text-slate-100">{l.symbol.replace('.L', '')}</span>
