...
export default function FTSEIndex() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const list = FTSE100.map(x => x.symbol)
  ...
  // 7d / 30d procent-verandering (batch, zelfde API als AEX)
  ...
  const summary = useMemo(() => {
    const withScore = FTSE100.map(a => ({ sym: a.symbol, s: scoreMap[a.symbol] })).filter(x => Number.isFinite(x.s))
    ...
    const pctArr = FTSE100.map(a => Number(quotes[a.symbol]?.regularMarketChangePercent))
  ...
  return (
    <>
      <Head><title>FTSE 100 Tracker — SignalHub</title></Head>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {FTSE100.map((row, i) => {
                    ...
                    const price = fmtPrice(q?.regularMarketPrice, q?.currency || 'GBP')
                    ...
                    <ul className="space-y-1 text-sm">
                      {summary.topGainers.map((g, i) => (
                        <li key={`g${i}`} className="flex justify-between">
                          <span className="text-gray-800">{g.symbol.replace('.L','')}</span>
                    ...
                    <ul className="space-y-1 text-sm">
                      {summary.topLosers.map((l, i) => (
                        <li key={`l${i}`} className="flex justify-between">
                          <span className="text-gray-800">{l.symbol.replace('.L','')}</span>
