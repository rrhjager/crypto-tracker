...
export default function Nikkei225PastPerformancePage({ rows, fetchError }: PageProps) {
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-extrabold">Nikkei 225 Past Performance</h1>
        </div>
                ...
                <tr key={r.symbol} className="hover:bg-white/[0.03] align-top">
                  <td className="px-3 py-2">
                    <Link href={`/stocks/${encodeURIComponent(r.symbol)}`} className="group block">
                      <div className="text-white/90 font-semibold truncate group-hover:text-white">{r.name}</div>
                      <div className="text-xs text-white/55 truncate group-hover:text-white/70">({r.symbol})</div>
                    </Link>
                  </td>
                  ...
                  <td className="px-3 py-2">
                    <div className="text-white/90 font-semibold tabular-nums">{r.lastSignal ? r.lastSignal.score : '—'}</div>
                  </td>
...
export const getServerSideProps: GetServerSideProps<PageProps> = async (ctx) => {
  try {
    ...
    const r = await fetch(`${baseUrl}/api/past-performance/nikkei225`, {
